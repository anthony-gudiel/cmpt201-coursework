...
port number). The port number is 8000 and the IP address is 127.0.0.1
2. Is it UDP or TCP? How do you know?
  TCP because socket() is using SOCK_STREAM. If it were UDP, it'd instead be
SOCK_DGRAM.
3. The client is going to send some data to the server. Where does it get this
data from? How can you tell in the code? The data is coming from STDIN_FILENO.
read() is getting the data from STDIN_FILENO as seen in the args, and write() is
using that result.
4. How does the client program end? How can you tell that in the code?
  The program ends when there is nothing to be read from STDIN_FILENO, as seen
in the line while ((num_read = read(STDIN_FILENO, buf, BUF_SIZE)) > 1) { write
}. Therefore, when the number of bytes read is less than or equal to 1, it goes
onto the rest of the code, which includes close() and exit()
*/
...
void *handle_client(void *arg) {
  ...
  while ((num_read = read(cfd, buf, BUF_SIZE)) > 0) {
    ...
    int tmp_count = ++total_message_count;
    pthread_mutex_unlock(&count_mutex);

    printf("Msg: %4d; Client ID %d: %s", tmp_count, client_id, buf);
  }
