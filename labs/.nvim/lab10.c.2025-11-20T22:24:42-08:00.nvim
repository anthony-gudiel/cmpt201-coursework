...
1. How is the client sending data to the server? What protocol?
  The client is using TCP, as seen by `SOCK_STREAM` in the `socket()` call.

2. What data is the client sending to the server?
  It is sending all the strings in the array named `messsages` using size
`BUF_SIZE`.

Understanding the Server:

1. Explain the argument that the `run_acceptor` thread is passed as an argument.
  `run_acceptor()` is passed a pointer to an `acceptor_args` struct so the
thread knows when to accept from clients, what to share with clients, and to
...
3. What does `main()` do with the received messages?
  Creates the shared list, calls `run_acceptor` to spawn threads that store
messages, and then traverses the list and prints each node's message if there
are enough messages.

4. How are threads used in this sample code?
  In `main()`, a single thread is created. This thread uses `run_acceptor()` to
accept new clients. It will also spawn more threads (TODO) to manage/add to the
shared linked list.
...
What sockets are made non-blocking?
  - The sockets in `run_client()` are made non-blocking when checking for
messages from the client.
  - The socket in `run_acceptor()` is made non-blocking when checking for
clients

- Why are these sockets made non-blocking? What is the purpose?
  These sockets are made non-blocking so that when they check for clients or
messages (respectively), and there are none, they don't get stuck and wait.
Otherwise, this could interrupt some of our other logic.
...
int main() {
  ...
  int min_messages = MAX_CLIENTS * NUM_MSG_PER_CLIENT;

  while (true) {
    pthread_mutex_lock(&list_mutex);
    int num_messages = list_handle.count;
    pthread_mutex_unlock(&list_mutex);

    if (num_messages >= min_messages) {
      break;
    }
  }
